...
## 第11章 与 cuBLAS 差距深度分析

> 本章不实现代码，而是系统性地拆解 cuBLAS 在每个层次上做了哪些我们没做的事情。

### 11.1 差距全景

| 层次 | cuBLAS 做法 | 我们当前做法 | 差距贡献 |
|------|------------|------------|---------|
| 计算单元 | Tensor Core TF32 | CUDA Core FP32 + WMMA TF32 | 已对齐（WMMA） |
| 指令调度 | 手写 SASS 汇编 | nvcc 编译的 PTX | **中等** |
| 数据加载 | cp.async 3+ stage pipeline | cp.async 2-stage DB | **中等** |
| Shared Memory | XOR swizzle + padding | 无 padding（BK=8）或对齐 padding | **中等** |
| 寄存器分配 | 手工寄存器分配，零 spill | 依赖编译器 | **中等** |
| 分块策略 | 动态启发式选择 TM/TN/BK | 固定 TM=8/TN=8/BK=16 | **小** |
| 精度 | Kahan 补偿求和 / 混合精度 | 纯 TF32（无补偿） | 小 |
| Kernel Launch | Persistent Kernel / CUDA Graph | 传统 launch | **小** |

### 11.2 SASS 级别差距：指令调度

cuBLAS 的 GEMM kernel 有手写的 SASS（Shader Assembly）代码。SASS 是 GPU 的最低级指令集，比 PTX 更底层，直接控制：
- **指令双发射**：Ampere SM 每个 cycle 可以同时发射 1 条 FP32 指令和 1 条 INT 指令（或 1 条 Load/Store 指令）。合理安排指令顺序可以最大化 dual-issue。
- **寄存器 Bank 冲突**：寄存器文件有 4 个 bank，同一 cycle 最多读 3 个源操作数。cuBLAS 手工安排寄存器分配避免 bank conflict。
- **指令延迟隐藏**：FFMA 有 4 cycle 延迟。cuBLAS 在 4 条 FFMA 之间穿插 Load 指令，完全隐藏计算延迟。

我们依赖 nvcc 的 `-O3` 优化，这已经是编译器的最佳努力，但编译器无法做到手写 SASS 级别的精确调度。

### 11.3 数据加载差距：多 Stage Pipeline

我们使用 2-stage double buffering：

```
[Load T0] → [Compute T0 + Load T1] → [Compute T1 + Load T2] → ...
```

cuBLAS 使用 **3-4 stage** pipeline：
...
多 stage 的优势：
- 即使 Global Memory 延迟有波动（DRAM refresh, bank collision），也有更多 buffer 吸收抖动
- 每个 stage 的 Shared Memory 更小 → 更多 Block 驻留 SM → 更高 Occupancy
- Pipeline 更深意味着 issue slot 填得更满

代价是寄存器和 Shared Memory 压力更大，需要更精细的资源管理。

### 11.4 Shared Memory 差距：Bank Conflict 消除

**Bank Conflict 回顾**：Shared Memory 有 32 个 bank，同一 cycle 只能服务 32 个不同的 bank。如果两个线程访问同一 bank 的不同地址，产生 conflict，访问被串行化。

**cuBLAS 的解决方案**：XOR Swizzle——对 Shared Memory 地址施加 XOR 变换：
```
swizzled_addr = addr ^ ((addr / 32) * P)
```
这打乱了连续地址到 bank 的映射，消除了特定 stride 下的 conflict。

**我们的状态**：BK=8 时，ld=8 意味着 bank 访问模式相对简单，bank conflict 不明显。BK=16 时 ld=16，某些 warp-level WMMA 访问可能产生 2-way bank conflict。BK=32 时 ld=32，bank conflict 急剧恶化（16 KB SMEM，32→16 way conflict 可能），导致性能反而下降。

### 11.5 分块策略差距：自适应 vs 硬编码

cuBLAS 在 launch 前会根据 M、N、K 的值选择最优的分块参数：

| 矩阵大小 | cuBLAS 可能的 TM/TN | BM/BN | BK |
|---------|-------------------|-------|-----|
| 256² | TM=4, TN=4 | 64 | 8 |
| 512² | TM=8, TN=8 | 128 | 16 |
| 4096² | TM=8, TN=8 | 128 | 32 |

我们固定使用 TM=8、TN=8、BM=128、BN=128，这在大矩阵时最优，但在小矩阵时 Block 数量不够填满 SM。例如 256² 只需要 4 个 Block，GPU 有 38 个 SM 大量空闲。

### 11.6 量化差距估算

以 4096² FP32 为例，cuBLAS 达到 11.0 TFLOPS，我们达到 7.6 TFLOPS：

| 失去的性能 | 大约来源 |
|-----------|---------|
| ~1.5 TFLOPS | SASS 级指令调度 + 寄存器分配 |
| ~1.0 TFLOPS | 3+ stage cp.async pipeline |
| ~0.5 TFLOPS | Shared Memory swizzle |
| ~0.3 TFLOPS | 其他（持久 kernel、CUDA Graph 等微优化） |
| ~0.1 TFLOPS | 动态分块 + 精度补偿 |

### 🔥 面试题 21：你的 GEMM 和 cuBLAS 差 30%，你会怎么继续追？

**话术回答**：

"差距是 30%（11 vs 7.6 TFLOPS），但这不是一个单一问题——是 4-5 个独立差距的叠加。我会按投入产出比排序：

**第一优先级：SASS 级调优（预期 +10-15%）**
用 `cuobjdump -sass` 反编译 cuBLAS 的 kernel，逐指令分析：
- 找出 dual-issue 的机会点（哪些 FFMA 之间有 load 指令可以穿插）
- 检查寄存器 bank conflict（用 Nsight Compute 的 Scheduler Statistics）
- 用 `asm volatile` 内联关键路径的 SASS 指令（不需要全部手写汇编）

**第二优先级：3-stage cp.async pipeline（预期 +5-8%）**
从 2-stage double buffer 升级到 3-stage。需要重新计算 Shared Memory 预算（3×8KB = 24KB vs 当前 2×16KB = 32KB，反而更省！因为每个 stage 更小）。关键是把 BK=16 拆成 BK=8 × 3 stage。

**第三优先级：LDS.128 和 Bank Conflict（预期 +3-5%）**
把 Shared Memory → Register 的标量加载改成 float4 向量化（`float4 v = *reinterpret_cast<float4*>(&As[...])`）。如果 Nsight Compute 显示 bank conflict，加 padding 或 XOR swizzle。

**第四优先级：分块自适应（预期 +2-5%，主要在小矩阵上）**
根据 M、N、K 的范围做 if-else 分发到不同的 kernel 实例（TM=4/8, BK=8/16）。这个好处是小矩阵性能大幅改善。

**不推荐的方向**：手写完整 SASS kernel。投入产出比太低——cuBLAS 的 SASS kernel 是 NVIDIA 团队多年的积累，单人去追意义不大。不如把上面的优化做好，接近 10 TFLOPS 已经是非常优秀的成绩。

**最重要的教训**：优化不是无脑堆技术——cp.async DB 加了但只提升 4%，WMMA TF32 加了也只提升 1%。这说明**瓶颈已经不在计算或单次加载，而在整个 pipeline 的平衡**。真正的高手不是在某个点极致优化，而是让加载、计算、写回三者完美 overlap，无一空闲。"
...
### 12.4 LDS.128 + `__launch_bounds__` 优化
...
## 第13章 Level 7 — WMMA + cp.async Double Buffering

### 13.1 动机

从 NCU 分析得知 WMMA TC 的瓶颈是 barrier 等待（92% warp 空闲），而非 Tensor Core 吞吐不足。将 cp.async DB 与 WMMA 合并，让 Global→Shared 的 A 加载异步进行，与 WMMA 计算重叠，减少 warp 空等时间。

### 13.2 实现要点

- A 矩阵加载使用 `__pipeline_memcpy_async`（非转置，16-byte 对齐满足）
- B 矩阵加载使用同步 float4 + 转置（`__pipeline_memcpy_async` 无法处理转置后的跨行地址）
- 其他结构同 sync WMMA kernel

### 13.3 效果

| Size | DB Async | WMMA TC | WMMA+DB | WMMA+DB vs WMMA |
|------|----------|---------|---------|-----------------|
| 4096² | 7.86 TF | 7.61 TF | 7.74 TF | +1.7% |
| 2048² | 7.07 TF | 7.27 TF | 7.17 TF | -1.4% |
| 1024² | 5.79 TF | 5.98 TF | 6.06 TF | +1.3% |
| 512² | 2.08 TF | 2.32 TF | 2.52 TF | **+8.6%** |

WMMA+DB 缩小了与 CUDA Core 的差距，但由于 B 加载仍是同步的，barrier 数量未减少（256 次），整体 `__syncthreads()` 开销仍然主导运行时。**真正降 barrier 需要 `cuda::barrier` 替代 `__syncthreads()`，这是 cuBLAS 的核心技术之一。**
...
| Level | Kernel | 核心技术 | TFLOPS | 累计 | 文件 |
|-------|--------|---------|--------|------|------|
| 1 | Naive | Global Memory | 1.0 | 1.0x | gemm_kernels.cu |
| 2 | Shared | SMEM 32×32 | 1.3 | 1.3x | gemm_kernels.cu |
| 3 | Float4 | LDG.128 | 1.3 | 1.3x | gemm_kernels.cu |
| 4 | RegTile | 64 acc, TM=TN=8 | 7.2 | 7.0x | gemm_kernels.cu |
| 5 | DB Async | cp.async, BK=16 | 7.5 | 7.4x | gemm_kernels_async.cu |
| 6 | WMMA TC | TF32 Tensor Core | 7.6 | 7.5x | gemm_kernels_tc.cu |
| 7 | LDS.128+LB | SMEM vec + bounds | 7.9 | 7.7x | gemm_kernels.cu |
| 8 | **WMMA+DB** | TC + cp.async | **7.7** | **7.6x** | gemm_kernels_tc_async.cu |
| — | cuBLAS | Tensor Core + SASS | 11.0 | 10.7x | 参考 |
...
│
├─ Register Tiling (64 累加器, TM=TN=8, BK=8)
│  7.2 TFLOPS, 44% (+450%)
...
│
├─ LDS.128 + __launch_bounds__
│  7.9 TFLOPS, 49% (+5%)
│  Shared Memory Load 指令减少 4x，IPC 达 1.97。**← CUDA Core 实际上限**
│
...
└─ WMMA + cp.async Double Buffering
   7.7 TFLOPS, 48% (+2%)
   A 加载异步化有微小改善，但 barrier 数量未变，warp 空闲仍 90%
...
7.86 TFLOPS（49% FP32 峰值）是手写 CUDA C++ GEMM 的优秀成绩。项目覆盖了从 Naive 到 Tensor Core 的 7 级优化、21 道面试题、NCU profiling、完整的 benchmark 框架，以及每一级优化 WHY 的深度分析。对简历面试、对深入理解 GPU 计算——这套项目已经做到了它该做的一切。

---

> **文档版本**: v6.0 | **日期**: 2026-05-20 | **GPU**: RTX 3060 Ti (Ampere, sm_86)
> 所有性能数据来自 `benchmark.py` 实测，NCU 数据来自 `run_ncu.bat` (--set full, 1024²)
> v6.0 更新：优化路径全览（14.2）、NCU 逐项对比（14.3）、差距拆解（14.4）、不可弥补原因（14.5）
